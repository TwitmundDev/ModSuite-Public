...
public class PaginatedGuiWarn {

    private String title;
    private int row;
    private List<WarnEntity> warns;


    public PaginatedGuiWarn(String title, int row, List<WarnEntity> warns) {
        this.title = title;
        this.row = row;
        this.warns = warns;
    }
    ...
    public PaginatedGui createGui() {
        ...
        PaginatedGui gui = Gui.paginated()
                ...
                .disableAllInteractions()
                .create();
            for (WarnEntity warn : warns) {
                gui.addItem(
                        ItemBuilder.from(Material.BOOK)
                                .setName(warn.getWarnReason())
                                .addLore(LangEnum.GUI_WARNED_BY.getValue().replace("%author%", warn.getPlayerAuthor().getPlayer_Name()))
                                .addLore(LangEnum.GUI_DATE.getValue() + DateFormatter.formatDate(warn.getWarnDate()))
                                .asGuiItem());
            }
